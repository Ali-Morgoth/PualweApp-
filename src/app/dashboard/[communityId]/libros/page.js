...
export default function Libros({ modoPublico = false }) {
  ...
  useEffect(() => {
    if (modoPublico) return; // no hace falta para modo público
  ...
  // Leer libros filtrados por categoría y communityId si no es público
  useEffect(() => {
    let q;
    ...
    if (modoPublico) {
      q = categoriaFiltro
        ...
        : query(collection(db, "libros"), orderBy("fecha", "desc"));
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, "libros"), {
        ...
        fecha: Timestamp.now(),
        communityId: communityId || null, // importante
      });
