...
export default function Tesoreria() {
  ...
  // Estados para subir archivo: loading y success
  const [isUploading, setIsUploading] = useState(false);
  const [uploadSuccess, setUploadSuccess] = useState(false);

  // Categorías para Tesorería
  ...
  // Subir documento a Firebase Storage y Firestore
  const handleSubmit = async (e) => {
    ...
    try {
      const storageRef = ref(
        storage,
        `tesoreria/${Date.now()}_${formData.archivo.name}`
      );
      ...
      // Después de mostrar éxito, limpiar y cerrar modal tras 2 seg
      setTimeout(() => {
  ...
  // Formatear fecha a formato local (es-CL)
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6 overflow-x-hidden">
        ...
        >
          {categoriasTesoreria.map((categoria) => (
      ...
      {modalOpen && (
        <>
          ...
          >
            ...
            {/* Si está subiendo, mostrar spinner */}
            {isUploading && (
            ...
            {/* Si uploadSuccess es true, mostrar mensaje de éxito */}
            {uploadSuccess && (
            ...
            {/* Si no está subiendo ni éxito, mostrar el formulario */}
